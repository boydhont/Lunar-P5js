...
let Render = {
	...
	tableCamera: (target, cameraDistance, cameraPosition) => {
		let mousePosition = new LVector(
			cameraDistance,
			map(cameraPosition.x, 0, 1, -PI, PI),
			map(cameraPosition.y, 0, 1, 0, PI / 2)
		);
		Render.orbitCamera(target, mousePosition);
	},
	...
	singleColourMesh: (mesh, firstColour) => {
		beginShape(TRIANGLES);
		for (let f of mesh.faces) {
			let vertices = [mesh.vertices[f.a], mesh.vertices[f.b], mesh.vertices[f.c]];
			let n = Vectors.unitVector(Vectors.crossProduct(Vectors.twoPointVector(vertices[0], vertices[2]), Vectors.twoPointVector(vertices[0], vertices[1])));
			...
			let grayscaleColour = (colour.x + colour.y + colour.z) / 3;
			let shadedColour = new LVector(
				(firstColour.x + grayscaleColour) / 2,
				(firstColour.y + grayscaleColour) / 2,
				(firstColour.z + grayscaleColour) / 2,
			);
			fill(shadedColour.x, shadedColour.y, shadedColour.z);
			for (let v of vertices) vertex(v.x, v.y, v.z);
		}
		endShape();
	},
	twoColourMesh: (mesh, firstColour, secondColour) => {
		beginShape(TRIANGLES);
		for (let f of mesh.faces) {
			let vertices = [mesh.vertices[f.a], mesh.vertices[f.b], mesh.vertices[f.c]];
			let n = Vectors.unitVector(Vectors.crossProduct(Vectors.twoPointVector(vertices[0], vertices[2]), Vectors.twoPointVector(vertices[0], vertices[1])));
			...
			let colour = new LVector(
				map(n.x, -1, 1, fC.x, sC.x),
				map(n.y, -1, 1, fC.y, sC.y),
				map(n.z, -1, 1, fC.z, sC.z)
			);
